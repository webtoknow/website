...
const TagsNav: React.FC<TagsNavProps> = ({ tags, showAllArticles, showAllTags, size = 'md', className, pathname }) => {
  return (
    <section className={clsx(styles.wrapper, className)}>
      ...
      {showAllTags && (
        <Link className={clsx(styles.all, size === 'sm' && styles.allSmall )} to="/blog/tags" title="Go to tags page">
          See all tags
        </Link>
      )}
    </section>
